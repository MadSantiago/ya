package p000;

import java.nio.ByteBuffer;

/* JADX INFO: renamed from: ۥۗ */
/* JADX INFO: compiled from: r8-map-id-0d494f8b6d83be2bf58e981e38c510150c6d0fab5a67cb4d4c794ec6fd37749a */
/* JADX INFO: loaded from: classes.dex */
public final class C0035 {

    /* JADX INFO: renamed from: ۥؗ */
    public int f6876;

    /* JADX INFO: renamed from: ۥُ */
    public final C4452 f6877;

    /* JADX INFO: renamed from: ۥۗ */
    public ByteBuffer f6878;

    /* JADX INFO: renamed from: ۥۣ */
    public int f6879;

    /* JADX INFO: renamed from: ۦؑ */
    public int f6880;

    public C0035() {
        C4452 c4452 = C4452.f14674;
        if (c4452 == null) {
            c4452 = new C4452(3);
            C4452.f14674 = c4452;
        }
        this.f6877 = c4452;
    }

    /* JADX INFO: renamed from: ۥؗ */
    public final void m4028(int i, ByteBuffer byteBuffer) {
        this.f6878 = byteBuffer;
        if (byteBuffer == null) {
            this.f6879 = 0;
            this.f6876 = 0;
            this.f6880 = 0;
        } else {
            this.f6879 = i;
            int i2 = i - byteBuffer.getInt(i);
            this.f6876 = i2;
            this.f6880 = this.f6878.getShort(i2);
        }
    }

    /* JADX INFO: renamed from: ۥُ */
    public final int m4029(int i) {
        int i2 = i + this.f6879;
        return this.f6878.getInt(i2) + i2 + 4;
    }

    /* JADX INFO: renamed from: ۥّ */
    public final int m4030(int i) {
        int i2 = i + this.f6879;
        return this.f6878.getInt(this.f6878.getInt(i2) + i2);
    }

    /* JADX INFO: renamed from: ۥۗ */
    public final int m4031(int i) {
        if (i < this.f6880) {
            return this.f6878.getShort(this.f6876 + i);
        }
        return 0;
    }

    /* JADX INFO: renamed from: ۥۣ */
    public final int m4032(int i) {
        return this.f6878.getInt(i) + i;
    }

    /* JADX INFO: renamed from: ۦؑ */
    public final String m4033(int i) {
        ByteBuffer byteBuffer = this.f6878;
        int i2 = byteBuffer.getInt(i) + i;
        int i3 = byteBuffer.getInt(i2);
        int i4 = i2 + 4;
        this.f6877.getClass();
        if (!byteBuffer.hasArray()) {
            if ((i4 | i3 | ((byteBuffer.limit() - i4) - i3)) < 0) {
                throw new ArrayIndexOutOfBoundsException(String.format("buffer limit=%d, index=%d, limit=%d", Integer.valueOf(byteBuffer.limit()), Integer.valueOf(i4), Integer.valueOf(i3)));
            }
            int i5 = i4 + i3;
            char[] cArr = new char[i3];
            int i6 = 0;
            while (i4 < i5) {
                byte b = byteBuffer.get(i4);
                if (b < 0) {
                    break;
                }
                i4++;
                cArr[i6] = (char) b;
                i6++;
            }
            int i7 = i6;
            while (i4 < i5) {
                int i8 = i4 + 1;
                byte b2 = byteBuffer.get(i4);
                if (b2 >= 0) {
                    int i9 = i7 + 1;
                    cArr[i7] = (char) b2;
                    while (i8 < i5) {
                        byte b3 = byteBuffer.get(i8);
                        if (b3 < 0) {
                            break;
                        }
                        i8++;
                        cArr[i9] = (char) b3;
                        i9++;
                    }
                    i7 = i9;
                    i4 = i8;
                } else if (b2 < -32) {
                    if (i8 >= i5) {
                        C1078.m2272("Invalid UTF-8");
                        return null;
                    }
                    i4 += 2;
                    C4773.m8127(b2, byteBuffer.get(i8), cArr, i7);
                    i7++;
                } else if (b2 < -16) {
                    if (i8 >= i5 - 1) {
                        C1078.m2272("Invalid UTF-8");
                        return null;
                    }
                    int i10 = i4 + 2;
                    i4 += 3;
                    C4773.m8148(b2, byteBuffer.get(i8), byteBuffer.get(i10), cArr, i7);
                    i7++;
                } else {
                    if (i8 >= i5 - 2) {
                        C1078.m2272("Invalid UTF-8");
                        return null;
                    }
                    byte b4 = byteBuffer.get(i8);
                    int i11 = i4 + 3;
                    byte b5 = byteBuffer.get(i4 + 2);
                    i4 += 4;
                    C4773.m8121(b2, b4, b5, byteBuffer.get(i11), cArr, i7);
                    i7 += 2;
                }
            }
            return new String(cArr, 0, i7);
        }
        byte[] bArrArray = byteBuffer.array();
        int iArrayOffset = byteBuffer.arrayOffset() + i4;
        if ((iArrayOffset | i3 | ((bArrArray.length - iArrayOffset) - i3)) < 0) {
            throw new ArrayIndexOutOfBoundsException(String.format("buffer length=%d, index=%d, size=%d", Integer.valueOf(bArrArray.length), Integer.valueOf(iArrayOffset), Integer.valueOf(i3)));
        }
        int i12 = iArrayOffset + i3;
        char[] cArr2 = new char[i3];
        int i13 = 0;
        while (iArrayOffset < i12) {
            byte b6 = bArrArray[iArrayOffset];
            if (b6 < 0) {
                break;
            }
            iArrayOffset++;
            cArr2[i13] = (char) b6;
            i13++;
        }
        int i14 = i13;
        while (iArrayOffset < i12) {
            int i15 = iArrayOffset + 1;
            byte b7 = bArrArray[iArrayOffset];
            if (b7 >= 0) {
                int i16 = i14 + 1;
                cArr2[i14] = (char) b7;
                while (i15 < i12) {
                    byte b8 = bArrArray[i15];
                    if (b8 < 0) {
                        break;
                    }
                    i15++;
                    cArr2[i16] = (char) b8;
                    i16++;
                }
                i14 = i16;
                iArrayOffset = i15;
            } else if (b7 < -32) {
                if (i15 >= i12) {
                    C1078.m2272("Invalid UTF-8");
                    return null;
                }
                iArrayOffset += 2;
                C4773.m8127(b7, bArrArray[i15], cArr2, i14);
                i14++;
            } else if (b7 < -16) {
                if (i15 >= i12 - 1) {
                    C1078.m2272("Invalid UTF-8");
                    return null;
                }
                int i17 = iArrayOffset + 2;
                iArrayOffset += 3;
                C4773.m8148(b7, bArrArray[i15], bArrArray[i17], cArr2, i14);
                i14++;
            } else {
                if (i15 >= i12 - 2) {
                    C1078.m2272("Invalid UTF-8");
                    return null;
                }
                byte b9 = bArrArray[i15];
                int i18 = iArrayOffset + 3;
                byte b10 = bArrArray[iArrayOffset + 2];
                iArrayOffset += 4;
                C4773.m8121(b7, b9, b10, bArrArray[i18], cArr2, i14);
                i14 += 2;
            }
        }
        return new String(cArr2, 0, i14);
    }

    /* JADX INFO: renamed from: ۦۙ */
    public C0035 m4034(int i) {
        C0035 c0035 = new C0035();
        int iM4031 = m4031(4);
        if (iM4031 == 0) {
            return null;
        }
        c0035.m4028(m4032((i * 4) + m4029(iM4031)), this.f6878);
        return c0035;
    }
}
